const input = `DMwrszrfMzSSCpLpfCCn
...
for (let i = 0; i<splittedInput.length; i+=3) {
    arr1 = splittedInput[i].split('')
    arr2 = splittedInput[i+1].split('')
    arr3 = splittedInput[i+2].split('')

    let badge = arr1.filter(x => arr2.includes(x) && arr3.includes(x))
    
    result += getPriority(badge[0])
}
